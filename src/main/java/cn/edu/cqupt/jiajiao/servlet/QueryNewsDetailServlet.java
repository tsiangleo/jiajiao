...
public class QueryNewsDetailServlet extends HttpServlet {

	/**
	 * Constructor of the object.
	 */
	public QueryNewsDetailServlet() {
		super();
	}
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		//获取新闻id
		String newsidstr = (String)request.getParameter("id");
		Long newsid = Long.parseLong(newsidstr);
		
		NewsService ns  = new NewsService();
		
		try {
			ns.updateClick(newsid);
		...
		try {
			newsinfo = ns.findNews(newsid);
		} catch (SQLException e) {
			e.printStackTrace();
		}
		request.setAttribute("newsinfo", newsinfo);
		RequestDispatcher rd = request.getRequestDispatcher("newsDetail.jsp");
